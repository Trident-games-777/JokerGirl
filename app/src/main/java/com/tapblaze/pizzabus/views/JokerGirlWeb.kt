...
import com.tapblaze.pizzabus.databinding.WebJokerGirlBinding
...
class JokerGirlWeb : AppCompatActivity() {
    ...
    private val jokerGirlWebViewModel by viewModels<JokerGirlViewModel> {
        JokerGirlViewModelFactory(application, ProtoRepository(jokerGirlPreferencesStore))
    }
    private var _binding: WebJokerGirlBinding? = null
    private val binding get() = _binding!!

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        _binding = WebJokerGirlBinding.inflate(layoutInflater)
        setContentView(binding.root)

        webView = findViewById(R.id.webView)
        webView.loadUrl(intent.getStringExtra(Const.EXTRA_LINK)!!)
    ...
    private inner class LocalClient : WebViewClient() {
        ...
        override fun onReceivedError(
            ...
            if (errorCode == -2) {
                Toast.makeText(this@JokerGirlWeb, "Error", Toast.LENGTH_LONG).show()
            }
        }
        ...
        override fun onPageFinished(view: WebView?, url: String?) {
            ...
            lifecycleScope.launch {
                jokerGirlWebViewModel.updateLink(url!!)
            }
        }
    }
}